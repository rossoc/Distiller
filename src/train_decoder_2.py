#!/usr/bin/env python3
"""
Training script for the Decoder2 (Vocabulary-based Decoder) using PyTorch Lightning.

This script trains the Decoder2 to predict vocabulary tokens from input embeddings.
The best model (lowest validation loss) and training configuration are saved to
a timestamped folder in outputs/.

Usage:
    python src/train_decoder_2.py --epochs 50 --batch_size 32

Output:
    outputs/decoder_2_YYYYMMDD_HHMMSS/
...
from model.decoder_2_trainer import Decoder2Trainer
from model.callback import BestModelSaveCallback
from data.decoder_2_datamodule import Decoder2DataModule
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Decoder2 (Vocabulary-based Decoder) with Lightning"
    )
    ...
    parser.add_argument(
        ...
        help="Path to GGUF model for tokenizer vocabulary",
    )

    # Model arguments
    parser.add_argument(
        "--vocab_size", type=int, default=256000, help="Vocabulary size"
    )
    parser.add_argument(
...
def main():
    ...
    logger = WandbLogger(
        ...
        name=args.run_name or "decoder_2",
