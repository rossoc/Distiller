...
class TokenExtractor:
    ...
    @staticmethod
    def _extract_from_gguf(model_path: Path) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
    ...
    def extract_sequence(
        ...
        # Handle EOS stopping
        if stop_at_eos:
            eos_positions = token_ids[ x == self.eos_token_id]
            eos_positions = [
                i for i, tid in enumerate(token_ids) if tid == self.eos_token_id
            ]
            if eos_positions:
    ...
    def get_confidence_stats(
        self, results: List[TokenExtractionResult]
    ) -> Dict[str, float]:
        """
