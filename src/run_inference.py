...
def run_predictions(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Predicting"):
            ...
            pooled_input = (input_embeddings * input_mask).sum(dim=1) / input_mask.sum(
                dim=1
            ).clamp(min=1)
            ...
            output = model(
                memory=pooled_input, memory_padding_mask=memory_padding_mask[:, :1]
            )
...
def main():
    ...
    test_dataset = EmbeddingDecoderDataset(
        X_test_list, y_test_list, encoder, args.max_length
    )
    ...
    retrieved_texts, scores = retrieve_with_faiss(
        predicted_embeddings, retriever, k=args.k
    )
